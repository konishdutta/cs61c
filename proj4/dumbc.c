...
int Matrix61c_set_subscript(Matrix61c* self, PyObject *key, PyObject *v) {
    ...
    Py_ssize_t row_start = boundaries[0];
    Py_ssize_t row_end = boundaries[1];
    Py_ssize_t col_start = boundaries[2];
    Py_ssize_t col_end = boundaries[3];
    Py_ssize_t row_step = boundaries[4];
    Py_ssize_t col_step = boundaries[5];
    Py_ssize_t row_length = boundaries[6];
    Py_ssize_t col_length = boundaries[7];
    ...
    // 1D Slice
    if (row_length == 1 || col_length == 1) {
        // check the list is a list of ints or floats
        if (!PyList_CheckExact(v)) {
            PyErr_SetString(PyExc_TypeError, "List was expected.");
            return -1;
        }
        // check size
        int size = (int) PyList_Size(v);
        if ((row_length == 1 && size != col_length) || (col_length == 1 && size != row_length)) {
            PyErr_SetString(PyExc_TypeError, "List is wrong length.");
            return -1;
        }
        // check everything in the list is an int
        for (int i = 0; i < size; i++) {
            PyObject *curr = PyList_GetItem(v, i);
            if (!PyFloat_Check(curr) && !PyLong_Check(curr)) {
                PyErr_SetString(PyExc_TypeError, "List contains non-numeric characters.");
                return -1;
            }
        }
        if (row_length == 1) {
            for (int i = 0; i < col_length; i++) {
                self->mat->data[row_start][col_start + i] = PyFloat_AsDouble(PyList_GetItem(v, i));
            }
        } else {
                for (int i = 0; i < row_length; i++) {
                    self->mat->data[row_start + i][col_start] = PyFloat_AsDouble(PyList_GetItem(v, i));
                }
        }
        return 0;
    }
    ...
    if (!PyList_CheckExact(v)) {
        PyErr_SetString(PyExc_TypeError, "2D List was expected.");
        return -1;
    }
    // check row size
    int size = (int) PyList_Size(v);
    ...
    for (int i = 0; i < size; i++) {
        ...
        // iterate through list and check for floats
        int list_size = (int) PyList_Size(curr);
        // first check for length
        if (list_size != col_length) {
            PyErr_SetString(PyExc_TypeError, "List is wrong length.");
            return -1;
        }
        for (int j = 0; j < list_size; j++) {
            PyObject *curr_num = PyList_GetItem(curr, j);
            if (!PyFloat_Check(curr_num) && !PyLong_Check(curr_num)) {
                PyErr_SetString(PyExc_TypeError, "List contains non-numeric characters.");
                return -1;
            }
        }
    ...
    for (int i = 0; i < row_length; i++) {
        for (int j = 0; j < col_length; j++) {
            self->mat->data[row_start + i][col_start + j] = PyFloat_AsDouble(PyList_GetItem(PyList_GetItem(v, i), j));
        }
    }
    return 0;
}
